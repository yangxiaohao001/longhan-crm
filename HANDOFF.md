...
## ⚡ 数据备份体系（2026-09-16 建立）

用户担心"系统出 bug 数据全丢"。现状与对策：

- **数据唯一live副本在 Supabase**（13 表）；Cloudflare 只承载程序，站点挂了数据不丢，可在 Cloudflare 控制台一键回滚任意历史版本。
- **已建立本机自动备份**：`D://Trae solo Demo  01\crm-backup\`
  - `backup_crm.py`：全量导出 13 张表为单个 JSON（保留最近 60 份，自动清理）
  - `restore_crm.py <文件名>`：把备份整包 upsert 回云端（需输入 YES 确认；恢复前务必先看备份时间，会覆盖备份之后的新数据）
  - 桌面 `备份CRM数据.bat`：手动备份按钮
  - WorkBuddy 自动化「龙瀚CRM每日自动备份」：每天 20:00 自动跑（依赖 WorkBuddy 桌面端在运行）
- 备份/恢复链路均已实测通过（13/13 表）。
- 恢复操作建议由 AI 协助执行；若用户自行恢复，务必先备份当前（坏）状态再恢复旧备份。
- 遗留风险：备份存在同一台电脑（硬盘坏=备份丢）；进阶方案是再加一个异地目标（网盘/另一台电脑），或升级 Supabase 付费版获得平台侧自动备份。
